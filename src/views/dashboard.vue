<template>
  <div class="dashboard-container">
    <el-row :gutter="20">
      <el-col :span="6">
        <el-card v-if="aiAnalysisData.systemOverview">
          <div class="card-content">
            ...
            <div class="info">
              ...
              <p class="number">
                {{ aiAnalysisData.systemOverview.totalUsers }}
              </p>
              <p class="subtitle-title">
                活跃用户：{{ aiAnalysisData.systemOverview.activeUsers }}
              </p>
            </div>
          </div>
        </el-card>
        ...
        <el-card v-if="aiAnalysisData.systemOverview">
          <div class="card-content">
            ...
            <div class="info">
              ...
              <p class="number">
                {{ aiAnalysisData.systemOverview.totalDiaries }}
              </p>
              <p class="subtitle-title">
                今日新增：{{ aiAnalysisData.systemOverview.todayNewDiaries }}
              </p>
            </div>
          </div>
        </el-card>
        ...
        <el-card v-if="aiAnalysisData.systemOverview">
          <div class="card-content">
            ...
            <div class="info">
              ...
              <p class="number">
                {{ aiAnalysisData.systemOverview.totalSessions }}
              </p>
              <p class="subtitle-title">
                今日新增：{{ aiAnalysisData.systemOverview.todayNewSessions }}
              </p>
            </div>
          </div>
        </el-card>
        ...
        <el-card v-if="aiAnalysisData.systemOverview">
          <div class="card-content">
            ...
            <div class="info">
              ...
              <p class="number">
                {{ aiAnalysisData.systemOverview.avgMoodScore }} / 10
              </p>
    ...
    <el-row style="margin-top: 20px" :gutter="20">
      ...
      <el-col :span="12">
        <el-card style="width: 100%">
          ...
          <div class="chart-content">
            <div
              v-if="aiAnalysisData.consultationStats"
              class="consultation-stats"
            >
              <div class="stat-item">
                <div class="stat-label">总会话数</div>
                <div class="stat-value">
                  {{ aiAnalysisData.consultationStats.totalSessions }}
                </div>
              </div>
              <div class="stat-item">
                <div class="stat-label">平均时长</div>
                <div class="stat-value">
                  {{ aiAnalysisData.consultationStats.avgDurationMinutes }}
                </div>
              </div>
              <div class="stat-item">
                <div class="stat-label">活跃用户</div>
                <div class="stat-value">
                  {{ aiAnalysisData.systemOverview.activeUsers }}
                </div>
              </div>
            </div>
            <div
...
const queryAnalysisOverview = () => {
  getAnalysisOverview().then((res) => {
    aiAnalysisData.value = res;
    initCharts();
  });
};
//初始化emotionChart
const initEmotionChart = () => {
  //dom节点不存在，图表实例的挂载就没有意义
  if (!emotionChartRef.value) {
    return;
  }
  //销毁之前的图表，当后端实时返回数据时，需要轮询后端接口重新渲染新的图表，这个方法会重复调用，所有需要销毁之前的图表
  if (emotionChart) {
    emotionChart.dispose();
  }
  emotionChart = echarts.init(emotionChartRef.value);
  //获取情绪趋势的数据
  const trendData = aiAnalysisData.value.emotionTrend;
  // 配置图表
  const option = {
    title: {
      text: "情绪趋势分析",
...
const initConsultationChart = () => {
  ...
  // 获取咨询趋势的数据
  const dailyTrend = aiAnalysisData.value.consultationStats.dailyTrend;
  const option = {
    title: {
      text: "咨询活动统计",
      textStyle: {
...
const initUserActivityChart = () => {
  ...
  //获取用户活跃度趋势的数据
  const activityData = aiAnalysisData.value.userActivity;
  const option = {
    title: {
      text: "用户活跃度趋势",
      textStyle: {
